...
**关联文档**：spec.md v1.7 / api.md / file_map.md v1.8
**覆盖范围**：全部 FR-001~FR-029；三模式流水线；SSE 实时通信层；数据库层

> 分层定位：质量、测试与可观测层。
> 重整入口：参见 `design_layers.md`；本文负责验证、诊断和可观测性，不应作为业务字段、状态枚举或 API 响应的唯一权威。
...
**禁止在集成测试中 mock 数据库**（参照 spec.md §4.2 数据完整性要求，数据库 mock 会掩盖真实约束冲突，历史上已导致生产事故）。
