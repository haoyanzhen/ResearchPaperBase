...
文档版本：v0.1
更新日期：2026-04-28
用途：按照 `design_layer_template.md` 对现有设计文档进行结构重整，作为后续修订入口。
...
| 分层 | 当前承载文档 | 权威性说明 |
| --- | --- | --- |
| 产品与领域层 | `spec.md`, `ADR_design.md` | `spec.md` 定义 FR；`ADR_design.md` 定义顶层架构意图 |
| 信息架构与用户流程层 | `ui_design.md`, `spec.md` | `ui_design.md` 定义页面与路由；`spec.md` 定义业务入口条件 |
| 状态机与业务流程层 | `ADR_design.md`, `spec.md`, `schema.sql`, `api.md` | 存在冲突；状态枚举应以 `schema.sql` 为准，待人工确认 |
| 数据模型层 | `schema.sql`, `spec.md` | `schema.sql` 自声明为数据层唯一权威契约 |
| API 契约层 | `api.md`, `qa_design.md` | `api.md` 是接口入口，但响应结构和错误码需统一 |
| UI 与交互层 | `ui_design.md` | 页面草稿完整度较高，但视觉系统、响应式、可访问性缺失 |
| 权限与安全层 | `spec.md`, `api.md`, `schema.sql`, `qa_design.md` | 有用户隔离和管理员规则，但缺少完整权限矩阵 |
| 质量与可观测层 | `qa_design.md` | 错误信封、健康检查、测试策略较完整，但含实现状态描述 |
| 运维部署与演进层 | `spec.md`, `file_map.md` | 运行环境有描述，部署/迁移/备份设计不足 |

文档修订原则：

- 数据字段、枚举、约束冲突时，先以 `schema.sql` 为候选权威。
- API 请求/响应冲突时，以人工确认后的 `api.md` 修订版作为权威。
- 需求含义以 `spec.md` 为权威，但 `spec.md` 不应重复定义可由 API/DB 约束表达的细节。
- 架构意图以 `ADR_design.md` 为权威，特别是三模式互斥、深度研究只读约定、RAG 数据库分层。
- QA 只定义验证和诊断机制，不应作为业务字段或接口最终来源。

## 1. 产品与领域层

来源：`spec.md` §1~§3、`ADR_design.md`。
...
来源：`ui_design.md`、`spec.md` FR-005/FR-010/FR-019。
...
来源：`ADR_design.md`、`spec.md`、`schema.sql`、`api.md`。
...
- `api.md` 示例仍包含 `draft`、`completed`、`cancelled`。
...
- `spec.md` FR-019 禁止跳过阶段。
- `api.md` 和 `ui_design.md` 仍保留 `skip`。
...
来源：`schema.sql` 为权威，`spec.md` 数据需求章节作为解释性材料。
...
- 主要页面线框已经存在。
- 部分页面在 `ui_design.md` 中标注为未实现或部分实现。
...
来源：`spec.md`、`schema.sql`、`api.md`、`qa_design.md`。
...
- `qa_design.md` 同时包含设计规范和实现现状，建议拆分为：
  - `qa_design.md`：规范。
...
来源：`spec.md` 运行环境、`file_map.md` 实现目录说明。
...
1. 先在 `design_gap_decisions.md` 中确认所有待决策项。
2. 再更新 `api.md`，统一响应 envelope、错误码、SSE 认证、缺失端点。
3. 更新 `ui_design.md`，删除或调整与确认结果冲突的按钮和页面状态。
4. 更新 `spec.md`，保留需求和验收，不重复定义已由 DB/API 承担的字段细节。
5. 更新 `schema.sql`，只在需要新增表或约束时变更。
6. 更新 `qa_design.md`，让测试和诊断对齐最终契约。
