...
| FR-026 | 深度研究模式-Graph-RAG检索 | 高 | 待开发 |
| FR-027 | 深度研究模式-对话式探讨 | 高 | 待开发 |
| FR-029 | 深度研究模式-研究历史记录 | 高 | 待开发 |
| FR-030 | 深度研究模式-Agent总结模块 | 中 | 待开发 |
...
- FR-026：深度研究模式-Graph-RAG检索
- FR-027：深度研究模式-对话式探讨（含三种子模式）
- FR-029：深度研究模式-研究历史记录
- FR-030：深度研究模式-Agent总结模块
...
**需求描述：**
系统应支持以**项目为单元**配置自动检索推送调度，调度器定期扫描 `projects` 表中 `auto_push=TRUE` 且 `next_push_at ≤ now()` 的项目，自动触发构建流程并推送。

**功能要求：**
- 在项目设置中启用/禁用自动推送（写入 `auto_push` 字段）
- 支持配置推送间隔（天），可选值：1 / 3 / 7 / 14 / 30 天
- 启用后系统自动计算并写入 `next_push_at`
- 每次推送完成后更新 `last_push_at`，并按间隔计算下一次 `next_push_at`
- 支持查看该项目的调度状态（下次推送时间、上次推送时间）
- 支持立即触发一次推送（不影响自动调度时间）
- 同一项目的定时推送任务与手动触发任务互斥，有任务运行时跳过本次调度
...
**验收标准：**
- 调度以项目为单元，不同项目调度互相独立
- 到达 `next_push_at` 时自动触发构建，误差在 5 分钟以内
- 推送完成后 `last_push_at` 和 `next_push_at` 正确更新
...
- 所有功能需求（FR-001至FR-027、FR-029至FR-030）均已实现
- 功能测试通过率100%
