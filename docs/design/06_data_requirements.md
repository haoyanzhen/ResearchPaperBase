...
#### 共享论文与 Project 私人资产边界

- 全局 `papers` 只表示可复用论文身份与基础元数据，允许被多个 Project 引用。
- Project-Paper 关联、评分、筛选状态、推送状态、AI 分析、PDF 文件、解析文本、向量索引、GraphML 图谱、Construction Run、Research Session、Review Run、Knowledge Version 和任务临时产物均属于 Project 私人资产。
- 用户触发 Project 私人资产清理时，数据层应删除或排队清理上述 Project 私人资产，但不得删除仍可被其他 Project 复用的全局论文身份与元数据。
- 导出产物属于用户主动生成的保留资产，默认不随 Project 私人资产清理删除；导出产物的删除、过期和重新生成由导出生命周期策略管理。
- 当前旧表结构若仍在 `papers` 中保存 `pdf_path`、`text_path` 或 `ai_analysis`，实现时不得将这些字段解释为跨 Project 共享资产；后续 `06_data_model.sql` 应将文件路径、解析文本状态和 AI 分析迁移或等效约束到 Project 私有资产范围。
...
**说明**：存储所有论文的可复用身份与基础元数据，每篇论文唯一，支持多课题关联。PDF、解析文本、AI 分析等 Project 派生资产应按 Project 私有范围保存；下列旧字段若仍存在，仅作为待迁移或兼容字段，不代表跨 Project 共享文件资产。
...
**说明**：保存研究主题与论文的关联关系以及论文在该研究主题中的评分和状态。该表属于 Project 私人资产，用户清理 Project 私人资产时可删除，不影响全局 `papers` 论文身份与基础元数据。
