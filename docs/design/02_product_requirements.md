...
| FR-009 | 基础层 | 全局界面与状态面板 | P0 | 课题选择栏、Agent 状态栏、论文库侧栏、观点广场入口 |
| FR-010 | 基础层 | Project 论文库查看与管理 | P0 | 查看、搜索、筛选、详情、移除当前 Project 的论文关联 |
...
| FR-026 | 深度研究模式 | 知识图谱浏览 | P1 | 只读浏览、探索、导出构建模式维护的知识图谱 |
...
**需求描述：**
系统应提供独立于三种 Agent 模式的全局界面管理能力，帮助用户随时选择课题、查看 Agent 状态、打开论文库和访问观点广场。

**功能范围：**
- 课题选择栏展示用户所有 Project，并支持展开/折叠。
- Agent 状态栏展示当前 Project 当前模式的运行状态、阶段和错误提示。
- 论文库面板展示当前 Project 论文库入口。
- 观点广场作为独立页面或全局入口打开。
- 面板状态不应破坏当前 Agent 运行上下文。
...
#### FR-010 Project 论文库查看与管理

**需求描述：**
用户应能查看和管理当前 Project 关联的论文库，且操作只影响当前 Project 的关联关系和课题相关属性。

**功能范围：**
- 展示论文统计、列表、详情、评分、有效性、推送状态和 AI 分析。
- 支持标题、摘要、作者、来源、评分、有效性、推送状态等筛选。
- 支持从当前 Project 移除论文关联。
- 支持清空当前 Project 的论文关联。
- 不允许因移除当前 Project 论文而删除全局 `papers` 记录。

**验收标准：**
- 论文库只展示当前 Project 关联论文。
- 移除论文后其他 Project 不受影响。
- 清空操作需要二次确认。
- 统计数据与当前 Project 关联关系一致。
...
**需求描述：**
用户应能在构建模式中补充系统未检索到但对课题重要的论文或资料。
...
**需求描述：**
用户应能在深度研究模式中只读浏览当前 Project 的知识图谱，以辅助发现论文、作者、概念和方法之间的关联。

**功能范围：**
- 展示图谱节点、边和统计信息。
- 支持搜索、过滤、缩放、拖拽和节点详情查看。
- 支持从图谱节点跳转到论文详情。
- 支持导出图谱数据。
- 不允许在深度研究模式触发图谱重建。
