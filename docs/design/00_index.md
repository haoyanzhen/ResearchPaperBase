...
| 分层 | 当前承载文档 | 权威性说明 |
| --- | --- | --- |
| 产品与领域层 | `01_functional_requirements.md`, `03_architecture_decisions.md` | `01_functional_requirements.md` 定义 FR；`03_architecture_decisions.md` 定义顶层架构意图 |
| 信息架构与用户流程层 | `04_information_architecture_ui.md`, `01_functional_requirements.md` | `04_information_architecture_ui.md` 定义页面与路由；`01_functional_requirements.md` 定义业务入口条件 |
| 状态机与业务流程层 | `05_state_workflow.md`, `03_architecture_decisions.md`, `01_functional_requirements.md`, `06_data_model.sql`, `07_api_contract.md` | `05_state_workflow.md` 定义 Run/Session 生命周期、流程式步骤、调度和知识库刷新流程；字段枚举仍需与 `06_data_model.sql` 统一 |
...
- 需求含义以 `01_functional_requirements.md` 为权威，但 `01_functional_requirements.md` 不应重复定义可由 API/DB/状态机约束表达的细节。
- 架构意图以 `03_architecture_decisions.md` 为权威，特别是 Project + Construction Workspace / Construction Run / Research Session / Review Run 实例模型、并发边界、深度研究只读约定、RAG 数据库分层。
- QA 只定义验证和诊断机制，不应作为业务字段或接口最终来源。

## 1. 产品与领域层

来源：`01_functional_requirements.md`、`03_architecture_decisions.md`。

产品目标：

- 构建课题组级学术论文研究平台。
- 以 Project 为核心单元，通过 Construction Workspace / Construction Run / Research Session / Review Run 管理构建、深度研究和综述能力，不采用 Project 三模式互斥。
...
| 类型 | 目标 | Agent 形态 | 数据写入 |
| --- | --- | --- | --- |
| Construction Workspace | 维护长期构建配置 | 每 Project 唯一配置容器 | 写检索词、自动更新设置，不写 Knowledge Version |
| Construction Run | 检索、筛选、下载、解析、分析、入库、邮件推送 | 线性流水线执行记录 | 写论文库、向量库、图谱和 Knowledge Version |
| Research Session | 基于已有论文库做 Graph-RAG 对话 | 持续对话循环 | 只写对话记录、引用和总结 |
| Review Run | 生成大纲、章节、审查、汇总导出 | 分阶段审查工作流 | 写综述大纲、章节、审查和导出产物 |

非目标或限制：

- 推荐模块禁止评论、回复、讨论和实时聊天。
- 深度研究模式不负责建立或更新论文库、向量库、图谱。
- 同一 Project 同时只允许一个 active Construction Run 写入知识库，但 Research Session / Review Run 可与构建并行运行。
...
来源：`04_information_architecture_ui.md`、`01_functional_requirements.md` FR-WORKSPACE 功能族。
...
- Topbar：Logo、当前 Project、系统级入口、设置、头像。
- Project Workspace：Project 选择栏、Run/Session 选择栏、主体流程栏、Project 信息面板、Run/Session 信息面板。
- Content Area：渲染当前 Construction Workspace、Construction Run、Research Session 或 Review Run。

核心用户流程：

1. 注册/登录。
2. 创建 Project，默认进入 Project Workspace，并打开该 Project 的 Construction Workspace。
3. 配置 LLM、学术数据库、邮件收件人；管理员配置系统默认项。
4. 启动构建流程，按阶段审查结果。
5. 构建完成后创建 Research Session 或 Review Run。
...
- 当前 Project 不存在可用 Knowledge Version 时，不允许新建 Research Session 或 Review Run。
- 当前 Project 有 active Construction Run 时，不阻止进入 Research Session 或 Review Run，但应提示 Project 默认知识库正在更新。
- 切换 Construction Workspace、Construction Run、Research Session 或 Review Run 不应删除已有对话、综述草稿或阶段记录。
...
来源：`05_state_workflow.md`、`03_architecture_decisions.md`、`01_functional_requirements.md`、`06_data_model.sql`、`07_api_contract.md`。

### 3.1 Project 状态

候选权威：`05_state_workflow.md`，待 `06_data_model.sql` 同步。

| 状态 | 含义 |
| --- | --- |
| `active` | Project 处于活跃研究状态，可被自动构建调度扫描 |
| `paused` | Project 暂停自动追踪，可手动构建和查看历史 |
| `archived` | Project 归档保留，默认只读 |
| `deleted` | Project 删除态，不可进入 Workspace |

规则：

- Project 不保存唯一当前 Agent 模式。
- Project 状态不表达 Agent 运行状态。
- Construction Run、Research Session、Review Run 的状态由各自实例记录维护。

### 3.2 Run/Session 状态

候选权威：`05_state_workflow.md`。

| 状态 | 含义 |
| --- | --- |
| `draft` | 已创建但未启动，或等待用户补充启动参数 |
| `running` | 正在执行任务 |
| `waiting_user` | 等待用户确认、选择或修改 |
| `completed` | 已成功完成 |
| `failed` | 失败且需要用户处理 |
| `cancelled` | 用户取消或系统安全中止 |
| `archived` | 已归档，不再继续执行 |

当前待同步：

- `07_api_contract.md` 需确认 Run/Session 状态枚举是否完全采用 `05_state_workflow.md`。
- `/tasks` 接口的状态与 Run/Session/Stage 状态边界仍需统一。
...
- `05_state_workflow.md` 已允许规则性 `skipped` 步骤；`07_api_contract.md` 和 `04_information_architecture_ui.md` 需统一跳过入口和权限。
...
来源：`08_security_permissions.md`、`01_functional_requirements.md`、`06_data_model.sql`、`07_api_contract.md`、`09_quality_observability.md`。
...
来源：`10_operations_deployment.md`、`01_functional_requirements.md` 运行环境、`../file_map.md` 实现目录说明。
...
4. 更新 `01_functional_requirements.md`，保留需求和验收，不重复定义已由 DB/API/状态机承担的字段细节。
