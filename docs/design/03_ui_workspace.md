...
当前模型是 Agent 任务启动时的显式运行参数；重新登录后可恢复上次选择。用户未选择当前模型时，任务启动前必须提示选择，不得静默回落到系统默认模型。
...
├──────────────────┬──────────────────────┬─────────────────────────────────┤
│ Profile          │ 收件邮箱             │ Notification Editor             │
│ LLM              │ + 添加               │ 邮箱                            │
│ Sources          │ ┌──────────────────┐ │ ┌─────────────────────────────┐ │
...
P09 承载 FR-004 的用户收件邮箱、通知偏好和测试邮件。系统 SMTP / 发件邮箱仍由管理员系统配置维护。
