...
| 用户设置 | 账户、LLM、数据源 | 保存配置、测试连接 | FR-002 至 FR-003 |
...
P1/P2 页面和完整线框不进入 P0 主线框；仅在第 9.2 节作为后续能力清单保留，避免第一阶段实现范围膨胀。
...
适配说明：本附录已从旧 UI Wireframes v1.5 收敛为 P0 主线框。P1/P2 页面不展示完整线框，只在 9.2 后续能力清单中保留。旧版遗留 UI 的处理原则如下：

| 处理方式 | UI 范围 | 原因 |
| --- | --- | --- |
| 保留并小修 | P01、P02、P03、P06、P07、P08、P10、P11、P13、P14、P16、P17-P21、P23、P26-P28、P35-P42、P43-P46 | 能支撑 P0 主流程，但需隐藏 P1/P2 控件或收紧领域边界 |
| 删除并重设计 | P15、旧 P22、旧 P30-P32 | Construction Workspace、补充论文请求和知识资产面板都是新版领域设计，旧线框不足以承载 |
| 移入后续清单 | 旧 P04、旧 P05、旧 P09、旧 P12、旧 P24、旧 P25、旧 P29、旧 P33、旧 P34、旧 P37 的 Run/Session 部分 | 属于 P1/P2 或旧版统一实例操作，不进入 P0 主线框 |
...
│                  │                                                         │
│ 构建状态         │ ┌─────────────────────────────────────────────────────┐ │
│ active run       │ │ Project A                           active   KV-12  │ │
│ no KV            │ │ 论文 128 | 最近构建 2026-05-12 | 最近活动 10:22     │ │
│                  │ │ 构建: CR-018 running | 进入后查看自动更新设置       │ │
│                  │ │ [进入] [编辑] [暂停] [归档] [删除]                 │ │
│                  │ └─────────────────────────────────────────────────────┘ │
│                  │ ┌─────────────────────────────────────────────────────┐ │
│                  │ │ Project B                           paused   无 KV  │ │
│                  │ │ 论文 0 | 最近构建 -- | 最近活动 2026-05-10          │ │
│                  │ │ 构建: 未启动 | 进入后先打开 Construction Workspace  │ │
│                  │ │ [进入] [编辑] [恢复] [归档] [删除]                 │ │
│                  │ └─────────────────────────────────────────────────────┘ │
└──────────────────┴─────────────────────────────────────────────────────────┘
```
...
| 通知设置 | FR-004, P1 | 用户收件人、偏好和测试邮件后置 |
...
总体结论：旧版页面体系可以保留为导航参考，但主线框必须隐藏 P1/P2 内容。Construction Workspace、知识资产面板和论文补充入口属于新版领域设计，旧版线框已删除并重画；邮件推送页面已改为空邮件保护强制启用。
...
| P1/P2 | FR-004, FR-010, FR-019, FR-029, FR-030, FR-036 | 旧版后续能力线框过早出现在主线框中。 | 移入 9.2 后续能力清单，不展示完整线框。 |
