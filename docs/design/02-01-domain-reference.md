...
| `ConstructionWorkspace / ConstructionRun` | 论文发现、筛选、补充、入库和知识库写入 | ConstructionWorkspace、ConstructionRun、SearchTermSet、SearchTermVersion、SelectedSearchTerms、AutoUpdateSearchTerms、DataSourcePolicy、ConstructionRunConfigSnapshot、CandidatePaper、DeduplicationDecision、PaperScreeningDecision、ManualPaperSupplementRequest、AutoConfirmationPolicy、ConstructionCheckpoint | 不负责 Research/Review 消费知识，也不作为只读知识资产入口 |
...
| `ResearchSession` | 深研对话、检索上下文、回复尝试和引用上下文 | ResearchSession、ResearchTurn、ResearchMessage、ResearchResponseAttempt、ResearchRetrievalContext、ResearchOutputPreference、ResearchSummary、ResearchCitationContext、ResearchSessionMetadata | 不修改论文库，不上传论文，不触发图谱写入 |
...
| `EmailPushJob` | 邮件推送范围、论文项和收件人发送结果 | EmailPushJob、EmailPushScope、EmailPushPaperItem、EmailPushRecipientResult | 邮件失败不回滚论文入库，不发送空邮件 |
...
| `UserDataSourceConfig` | 配置 | Account / Config | 用户私有论文数据源配置 | FR-003 |
...
| `EmailPushJob` | 任务对象 | EmailPushJob | 论文邮件推送任务 | FR-004, FR-009, FR-027 |
...
- `KnowledgeVersion` 只能由成功 `ConstructionRun` 发布；失败、取消或未发布中间写入不得污染已发布版本。
...
- 邮件推送只包含当前 Project 中未推送且用户有权限接收的有效论文；空邮件不得发送，发送失败不得回滚论文入库。
...
| FR-012 | Project / Workspace, ProjectPaper, KnowledgeVersion / Evidence | ProjectKnowledgeAssetPanel、ProjectPaper、DocumentAccessRef、ProjectGraphView |
...
| FR-021 | ConstructionWorkspace / ConstructionRun | SelectedSearchTerms、AutoUpdateSearchTerms、CandidatePaper |
| FR-022 | ConstructionWorkspace / ConstructionRun, PaperIdentity, ProjectPaper | CandidatePaper、DeduplicationDecision、PaperScreeningDecision |
| FR-023 | ConstructionWorkspace / ConstructionRun, PaperIdentity, ProjectPaper | ManualPaperSupplementRequest、ExternalPaperIdentifier、DocumentAsset |
| FR-024 | ProjectPaper | DocumentAsset、DocumentProcessingState、TextSourceKind |
| FR-025 | ProjectPaper | PaperAnalysis |
| FR-026 | ConstructionWorkspace / ConstructionRun, KnowledgeVersion / Evidence | KnowledgeSyncState、KnowledgeVersion、GraphEntityRef |
| FR-027 | EmailPushJob, ProjectPaper | EmailPushJob、EmailPushPaperItem、ProjectPaper |
...
| FR-031 | ResearchSession, KnowledgeVersion / Evidence | ResearchCitationContext、CitationEvidence、GraphEntityRef |
| FR-032 | ReviewRun, KnowledgeVersion / Evidence | ReviewRunBrief、ReviewOutline、ReviewOutlineAssessment |
| FR-033 | ReviewRun, KnowledgeVersion / Evidence | ReviewChapterEvidenceContext、ReviewChapter、CitationEvidence |
